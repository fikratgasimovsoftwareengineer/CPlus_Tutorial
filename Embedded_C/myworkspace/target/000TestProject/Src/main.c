/**
 ******************************************************************************
 * @file           : main.c
 * @author         : Auto-generated by STM32CubeIDE
 * @brief          : Main program body
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2022 STMicroelectronics.
 * All rights reserved.
 *
 * This software is licensed under terms that can be found in the LICENSE file
 * in the root directory of this software component.
 * If no LICENSE file comes with this software, it is provided AS-IS.
 *
 ******************************************************************************
 */

#include <stdint.h>

#if !defined(__SOFT_FP__) && defined(__ARM_FP)
  #warning "FPU is not initialized, but the project is compiling for an FPU. Please initialize the FPU before use."
#endif

int main(void)
{
	// Pointer for Register CLock
	uint32_t *pClockCtrReg = (uint_32*) 0x40023830;

	//GPIO Mode Register
	uint32_t *pPortModeReg = (uint_32*) 0x40020C00;

	// GPIO Output Data Register to write
	uint32_t *pOutDataReg = (uint_32*) 0x40020C14;

	//1. Enable clock for GPIO
	// store temp val;
	uint32_t temp = *pClockCtrReg; //read operation and AND operation
	temp = temp | 0x00000008; // mask Created according to OR to set to 1
	*pClockCtrReg = temp;
	//======================================

	// A. Configure Mode of GPIO Pin as output
	//First Clear 24th and 25th position
	 uint32_t temp_2  *pPortModeReg;

	 // Mask of GPIO Mode Register
	 temp_2 = temp_2 & 0xFCFFFFFF;

	 //Set to ) First
	 *pPortModeReg = temp_2;
	 //=================================

	 // B. Set 25th to 0 and 24th of bits to 1
	 uint32_t temp_3 = *pPortModeReg;
	 temp_3 = temp_3 | 0x01000000;
	 *pPortModeReg = temp_3;

	 // GPIO port Output Data Register

	 uint32_t temp_4 = *pOutDataReg;
	 temp_4 = temp_4 | 0x1000;

	 *pOutDataReg = temp_4;


	printf("Hello World From MICROController: \n");
    /* Loop forever */
	for(;;);
}
